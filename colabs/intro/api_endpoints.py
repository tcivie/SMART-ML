...
def switch_traffic_light_program(tls_id, session_id, program_id, make_step=1,*,forced=False):
    ...
    response = requests.post(url, json={'session_id': session_id, 'program_id': program_id, 'make_step': make_step,'forced':forced},headers=HEADERS)
    if response.ok:
        return response.json()['simulation_metrics']
    return {}
...
def step_simulation(session_id, steps=1, tls_ids=None):
    ...
    url = f'{BASE_URL}/step'
    response = requests.post(url, json={'session_id': session_id, 'steps': steps, 'show_data_for_tls_ids': tls_ids},headers=HEADERS)
    return response.json()['simulation_metrics']
